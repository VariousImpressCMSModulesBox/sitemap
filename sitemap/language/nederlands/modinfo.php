<?php
/**
* Sitemap - Automatically sitemapping module for ImpressCMS
*
*
* File: language/nederlands/modinfo.php
*
* @copyright		http://www.xoops.org/ The XOOPS Project
* @copyright		XOOPS_copyrights.txt
* @copyright		http://www.impresscms.org/ The ImpressCMS Project
* @license			GNU General Public License (GPL)
*					a copy of the GNU license is enclosed.
* ----------------------------------------------------------------------------------------------------------
* @package		Sitemap 
* @since		1.30
* @author		chanoir
* ----------------------------------------------------------------------------------------------------------
* 				Sitemap
* @since		1.40
* @author		McDonald
* @version		$Id$
*/

define('_MI_SITEMAP_NAME', 'Sitemap');
define('_MI_SITEMAP_MESSAGE', 'Bericht');

define('_MI_SITEMAP_ADMENU_TOP', 'TOP' ) ;
define('_MI_SITEMAP_ADMENU_MYBLOCKSADMIN', 'Blokken/Groepen' ) ;

define('_MI_SITEMAP_BLOCKNAME', 'Sitemap');
define('_MI_SITEMAP_BLOCKDESC', 'Laat sitemap zien als een blok.');

define('_MI_MESSAGE', 'Bericht');
define('_MI_MESSAGEEDSC', '');

define('_MI_SHOW_SUBCATEGORIES', 'Laat Sub categoriëen zien');
define('_MI_SHOW_SUBCATEGORIESDSC', '');

define('_MI_INVISIBLE_WEIGHTS', 'Gewichten voor verbergen modulen');
define('_MI_INVISIBLE_WEIGHTSDSC', 'Modules welke het nummer als in de Module Administratie hebben zullen onzichtbaar zijn. De nummers dienen gescheiden te zijn met een komma.<br />(Standaard 0 of leeg)');

define('_MI_INVISIBLE_DIRNAMES', 'Folder namen of te verbergen modulen');
define('_MI_INVISIBLE_DIRNAMESDSC', "Specificeer de naam van de modulen die verborgen moeten zijn.<br />Folder namen dienen met een komma gescheiden te worden.<br />(bv xoopsheadline,newbb");

define('_MI_SELECT_STYLE', 'Kies een stijl');
define('_MI_SELECT_STYLE_DSC', 'Kies een stijl voor Sitemap.');
define('_MI_SELECT_DEFAULT', 'Standaard');
define('_MI_SELECT_SLICKMAP', 'Slickmap');

define('_MI_WARNING_FINAL', 'Deze module komt zoals hij is, zonder welke garantie dan ook. Ondanks dat de status van deze module geen beta is, is de module nog steeds in actieve ontwikkeling. Deze uitgave kan worden gebruikt in een actieve/online website of in een productieve omgeving, maar het gebruik valt volledig onder uw eigen verantwoordelijkheid, dit betekent dat de auteur niet verantwoordelijk is.');
define('_MI_WARNING_RC', 'Deze module komt zoals hij is, zonder welke garantie dan ook. Deze module is een uitgave kandidaat en dient niet te worden gebruikt in een actieve/online website of in een productieve omgeving. De module is nog steeds in actieve ontwikkeling en het gebruik ervan valt onder uw eigen verantwoordelijkheid, dit betekent dat de auteur niet verantwoordelijk is.');

define('_MI_SITEMAP_MODULE_GO','Start module');

define('_MI_CHANGEFREQ', 'Interval');
define('_MI_CHANGEFREQ_DSC', 'Hoe vaak verandert de pagina. Deze waarde bevat algemene informatie voor zoekmachines en kan niet precies overeenkomen met hoe vaak ze de pagina bezoeken. Voor meer informatie:<a href="http://www.sitemaps.org/protocol.php#changefreqdef" target="_blank">Sitemap.org</a><br />Deze instelling is alleen voor het Google sitemap bestand!');
define('_MI_CHANGEFREQ_HOURLY', 'per uur');
define('_MI_CHANGEFREQ_DAILY', 'dagelijks');
define('_MI_CHANGEFREQ_WEEKLY', 'wekelijks');
define('_MI_CHANGEFREQ_MONTHY', 'maandelijks');
define('_MI_CHANGEFREQ_YEARLY', 'jaarlijks');
?>